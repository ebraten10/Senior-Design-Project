%EOG-Based Communication System for Patients with Locked-in Sydrome
%Senior Design 2021-2022
%Author: Emma Braten

%This code was used to plot the average of ROC curve across all eye
%eye movements (B,U,D,R,L) for each subject in the public dataset found at
%https://www.um.edu.mt/cbc/ourprojects/EOG/EOGdataset. You must have the
%folder DATASET in the same folder as this m file for the code to run.

clc
clear

%Each value in numz is multiplied by the max/min value for each movement. 
% This results in a range of values from 0 to the max/min value that are
% used to find the TPR and FPR 
numz=0:0.01:1;

%TPR and FPR are cell arrays in which each cell contains a matrix of TPR or
%FPR values for all eye movements per each subject.
TPR = cell(1,6); 
FPR = cell(1,6);

%avgTPR and avgFPR are matrices with the average TPR and FPR values per
%subject across all eye movements. They are used to plot the average ROC
%curves for each subject
avgTPR = zeros(6,length(numz));
avgFPR = zeros(6,length(numz));

%AUC is a vector that saves the AUC score for each of the 6 subjects
AUC=zeros(1,6);
Hangle=4.4;
Vangle=2.6;

for s=1:6
    count=1;
    for mult=numz

        roothpath = sprintf('DATASET/S%d',s);
        load(fullfile(roothpath,'EOG.mat'))
        load(fullfile(roothpath,'TargetGA.mat'))
        load(fullfile(roothpath,'ControlSignal.mat'))
    
        T = TargetGA;
        H = EOG(1,:);
        V = EOG(2,:);
        dh = diff(H);
        dv = diff(V);
        C = ControlSignal;
    
        %id is a vector containing the start and end points of each forward
        %saccade, return saccade, and blink interval in the EOG data according
        %to the Control Signal (C). There are 300 trials that each contain
        %three separate intervals which makes id a vector of 899 points + the
        %additional starting point (1) and ending point (length(dh)) for a
        %total of 901 points.
        f = find((diff(C)==1) | (diff(C)==-2));
        id=[1,f,length(dh)]; 
    
        %Get the max/min values of each movement
        [thPH,thNH,thPV,thNV,thB] = MinAndMax(H,V,C,T);
    
        %ACCB is the vector that contains a 1 at the indices that correspond to
        %intervals where a blink actually happens
        ACCB = zeros(1,900);
        indx = 1;
        for i=3:3:900
            ACCB(i)=1;
        end
    
        %ACC is the vector that contains a 1 at the indices that correspond to
        %intervals where a saccadic movement actually happens
        %Row 1 - Right
        %Row 2 - Left
        %Row 3 - Up
        %Row 4 - Down
        ACC = zeros(4,300);
        for Tcount=1:2:599
            if T(Tcount,1) > Hangle
                ACC(1,indx) = 1;
            else
                ACC(1,indx) = 0;
            end
            if T(Tcount,1) < -Hangle
                ACC(2,indx) = 1;
            else
                ACC(2,indx) = 0;
            end
            if T(Tcount,2) > Vangle
                ACC(indx) = 1;
            else
                ACC(indx) = 0;
            end
            if T(Tcount,2) < -Vangle
                ACC(4,indx) = 1;
            else
                ACC(4,indx) = 0;
            end
            indx = indx+1;
        end
    
    
        Pred = zeros(4,300);
        PredB = zeros(1,900);
        intv = 1;
        intvB = 1;
    
        for i=1:900
            [pksB,locsB] = findpeaks(dv(id(i):id(i+1)),id(i):id(i+1),'MinPeakHeight',thB*mult);
            %Check if a blink was detected in each interval
            if ~isempty(pksB)
                PredB(intvB) = 1;
            else
                PredB(intvB) = 0;
            end
            intvB = intvB+1;
    
            %Predicted saccadic movements are only checked during intervals of
            %forward saccades
            if rem(i,3)==1
    
                [pksH,locsH] = findpeaks(dh(id(i):id(i+1)),id(i):id(i+1),'MinPeakHeight',thPH*mult);
                [NpksH,NlocsH] = findpeaks(-dh(id(i):id(i+1)),id(i):id(i+1),'MinPeakHeight',thNH*mult);
                [pksV,locsV] = findpeaks(dv(id(i):id(i+1)),id(i):id(i+1),'MinPeakHeight',thPV*mult);
                [NpksV,NlocsV] = findpeaks(-dv(id(i):id(i+1)),id(i):id(i+1),'MinPeakHeight',thNV*mult);
    
                if ~isempty(pksH)
                    Pred(1,intv) = 1;
                else
                    Pred(1,intv) = 0;
                end
    
                if ~isempty(NpksH)
                    Pred(2,intv) = 1;
                else
                    Pred(2,intv) = 0;
                end
    
                if ~isempty(pksV)
                    Pred(3,intv) = 1;
                else
                    Pred(3,intv) = 0;
                end
          
                if ~isempty(NpksV)
                    Pred(4,intv) = 1;
                else
                    Pred(4,intv) = 0;
                end
                intv = intv+1;
            end   
        end
    
        %TPR and FPR values for each of the 5 movements are stored in cells
        %TPR{1,subject} and FPR{1,subject}
        %Row 1 - Right
        %Row 2 - Left
        %Row 3 - Up
        %Row 4 - Down
        %Row 5 - Blink
        for m=1:4
            [tpr,fpr]=roc(ACC(m,:),Pred(m,:));
            TPR{1,s}(m,count)=max(tpr);
            FPR{1,s}(m,count)=max(fpr);
        end
        [tprB,fprB]=roc(ACCB,PredB);
        TPR{1,s}(5,count)=max(tprB);
        FPR{1,s}(5,count)=max(fprB);
        count=count+1;
    end

    %Save the average of all TPR and FPR values per threshold per subject
    avgTPR(s,:) = mean(TPR{1,s},1);
    avgFPR(s,:) = mean(FPR{1,s},1);
end

x=[0.38,0.82,0.38,0.82,0.38,0.82];
y=[0.68,0.68,0.38,0.38,0.08,0.08];

%Plot the average ROC curves and AUC scores for each subject
for sub=1:6
    AUC(1,sub)=trapz(flip(avgFPR(sub,:)),flip(avgTPR(sub,:)));
    subplot(3,2,sub)
    plot(avgFPR(sub,:),avgTPR(sub,:))
    ti=sprintf('S%d ROC Curve',sub);
    title(ti)
    xlabel("False Positive Rate")
    ylabel("True Positive Rate")
    str=sprintf('AUC = %f',AUC(1,sub));
    annotation('textbox',[x(sub) y(sub) .1 .1],'String',str,'FitBoxToText','on')
end

%Plot the average ROC curve over all subjects over all movements
figure
totalAvg(1,:) = mean(avgTPR,1);
totalAvg(2,:) = mean(avgFPR,1);
TotalAUC=trapz(flip(totalAvg(2,:)),flip(totalAvg(1,:)));
plot(avgFPR(sub,:),avgTPR(sub,:))
title('Average ROC Curve');
xlabel("False Positive Rate")
ylabel("True Positive Rate")
str=sprintf('AUC = %f',TotalAUC);
annotation('textbox',[.5 .3 .1 .1],'String',str,'FitBoxToText','on')